...
def parse_subtask(s):
    ...
    datetime_re = re.compile(r'([012]?\d:\d\d [0123]\d\.[01]\d\.\d\d\d\d)')
    status_re = re.compile(r'\[\s*(\w+)\s*\]')

    status_list = status_re.findall(s)
    if len(status_list) > 0:
        result['status'] = status_list[0]

    datetime_list = map(ts_to_datetime, datetime_re.findall(s))
